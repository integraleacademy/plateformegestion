...
function statFit(value){return /^[\d\s.,%€h+−-]+$/.test(String(value))?'metric':'meta'}
...
function statCards(content,prefix='new-stat',limit=4){return contentStats(content).slice(0,limit).map((item,index)=>`<section data-layout-role="${prefix}-${index}"><small>${esc(item.label)}</small><strong data-fit="${statFit(item.value)}" data-element-name="Valeur">${esc(item.value)}</strong></section>`).join('')}
function stepCards(content,prefix='new-step',limit=4){return contentSteps(content).slice(0,limit).map((item,index)=>`<li data-layout-role="${prefix}-${index}"><b>${String(index+1).padStart(2,'0')}</b><span data-fit="meta">${esc(item.title||item)}</span><small data-fit="body">${esc(item.text||'')}</small></li>`).join('')}
function moduleCards(content,prefix='new-module',limit=6){return contentModules(content).slice(0,limit).map((item,index)=>`<section data-layout-role="${prefix}-${index}"><b>${esc(item.duration||String(index+1).padStart(2,'0'))}</b><strong data-fit="meta">${esc(item.title||item)}</strong></section>`).join('')}
...
export function renderNewTemplateBody(ctx){
  ...
  const bodies={
    ...
    new_quote_window:`<main class="new-design new-quote-window" data-layout-role="new-quote-window"><span class="new-quote-mark" data-studio-decorative="true" aria-hidden="true">“</span>${quote}<footer>${highlight}<cite>INTÉGRALE ACADEMY</cite></footer></main>`,
    ...
    new_learning_map:`<main class="new-design new-learning-map" data-layout-role="new-learning-map"><header>${title}${intro}</header><div>${contentSteps(content).slice(0,4).map((item,index)=>`<section data-layout-role="new-map-node-${index}"><b>${index+1}</b><strong data-fit="meta">${esc(item.title||item)}</strong></section>`).join('')}</div></main>`,

    new_number_monolith:`<main class="new-design new-monolith" data-layout-role="new-number-monolith"><section><strong data-fit="${statFit(first.value)}" data-element-name="Valeur">${esc(first.value)}</strong><small>${esc(first.label)}</small></section><aside>${title}${intro}</aside></main>`,
    ...
    new_proof_rings:`<main class="new-design new-proof-rings" data-layout-role="new-proof-rings"><section><i></i><strong data-fit="${statFit(first.value)}" data-element-name="Valeur">${esc(first.value)}</strong><small>${esc(first.label)}</small></section><aside>${title}${statCards(content,'new-ring-stat',3)}</aside></main>`,
    new_metric_tower:`<main class="new-design new-tower" data-layout-role="new-metric-tower"><aside>${title}</aside><div>${contentStats(content).slice(0,4).map((item,index)=>`<section data-layout-role="new-tower-stat-${index}" style="--level:${index+1}"><b data-fit="${statFit(item.value)}" data-element-name="Valeur">${esc(item.value)}</b><span>${esc(item.label)}</span></section>`).join('')}</div></main>`,
    new_success_stamp:`<main class="new-design new-success-stamp" data-layout-role="new-success-stamp"><div><span>RÉSULTAT</span><strong data-fit="${statFit(first.value)}" data-element-name="Valeur">${esc(first.value)}</strong><small>${esc(first.label)}</small></div>${title}${intro}</main>`,
    new_barcode_stats:`<main class="new-design new-barcode" data-layout-role="new-barcode-stats"><header>${title}</header><div>${contentStats(content).slice(0,4).map((item,index)=>`<section data-layout-role="new-bar-${index}"><i style="--bar:${45+index*15}%"></i><strong data-fit="${statFit(item.value)}" data-element-name="Valeur">${esc(item.value)}</strong><small>${esc(item.label)}</small></section>`).join('')}</div></main>`,
    new_review_quote:`<main class="new-design new-review" data-layout-role="new-review-quote"><span>★★★★★</span>${quote}<footer><b data-fit="${statFit(first.value)}" data-element-name="Valeur">${esc(first.value)}</b><small>${esc(first.label)}</small></footer></main>`,
    ...
    new_data_wave:`<main class="new-design new-data-wave" data-layout-role="new-data-wave"><header>${title}${intro}</header><svg viewBox="0 0 720 220"><path d="M0 180 C110 190 120 55 240 95 S360 185 470 65 S610 95 720 18"/></svg><footer><strong data-fit="${statFit(first.value)}" data-element-name="Valeur">${esc(first.value)}</strong><span>${esc(first.label)}</span><b data-fit="${statFit(second.value)}" data-element-name="Valeur">${esc(second.value)}</b></footer></main>`,
    new_badge_wall:`<main class="new-design new-badge-wall" data-layout-role="new-badge-wall"><section>${title}</section><div>${contentStats(content).slice(0,5).map((item,index)=>`<span data-layout-role="new-badge-${index}"><b data-fit="${statFit(item.value)}" data-element-name="Valeur">${esc(item.value)}</b>${esc(item.label)}</span>`).join('')}</div></main>`,
    ...
    new_question_hook:`<main class="new-design new-hook" data-layout-role="new-question-hook"><span data-studio-decorative="true" aria-hidden="true">?</span>${edit('question',val(content,'question','Prêt à changer de métier ?'),'h1','new-title')}${intro}<button>${cta}</button></main>`,
