...
def generate_aps_trainer_contract_pdf(session_data, contract, output_path):
    ...
    cover_cards = [
        [card("Centre de formation", "Intégrale Academy<br/>54 chemin du Carreou<br/>83480 Puget-sur-Argens<br/>SIRET : 840 899 884 00026<br/>Représentant légal : Monsieur Clément VAILLANT"), card("Formateur / prestataire", f"{contract.get('trainerName')}<br/>{contract.get('status') or 'Statut juridique à compléter'}<br/>SIRET : {contract.get('siret') or 'à compléter'}<br/>{contract.get('address') or 'Adresse à compléter'}<br/>{contract.get('trainerEmail') or 'Email à compléter'}<br/>{contract.get('trainerPhone') or 'Téléphone à compléter'}")],
        [card("Mission", f"{formation_name} — {session_name}<br/>Du {start_date} au {end_date}<br/>Examen : {exam_date}<br/>Modalité : {modality_label}<br/>Volume : {float(contract.get('calculatedHours') or 0):g} h"), card("Rémunération", f"{float(contract.get('billedDays') or 0):g} jour(s) facturé(s)<br/>Tarif journalier HT : {_money(contract.get('dailyRate'))}<br/>Total HT : {_money(total_ht)}<br/>{tva_label}<br/>Total TTC : {_money(contract.get('totalTTC') or total_ht)}")],
        [card("Lieu d’intervention", "Intégrale Academy<br/>54 chemin du Carreou<br/>83480 PUGET-SUR-ARGENS"), card("Documents contractuels", "Planning détaillé des interventions<br/>Récapitulatif financier<br/>Engagement qualité et traçabilité pédagogique")],
    ]
