...
- `YOUSIGN_BASE_URL` (optionnel, alias historique accepté : `YOUSIGN_API_BASE_URL`) : URL de l'API, `https://api.yousign.app/v3` par défaut. Utiliser `https://api-sandbox.yousign.app/v3` pour les tests sandbox et vérifier que la clé API appartient au même environnement.
...
- `YOUSIGN_WORKSPACE_ID` (optionnel) : workspace Yousign dans lequel créer la demande lorsque la clé API est restreinte à un workspace.

Workflow fiche formateur : compléter au minimum le nom, le prénom, l'adresse postale et le SIRET, puis utiliser **Générer le contrat** ou directement **Générer et envoyer pour signature**. L'application produit le contrat cadre de sous-traitance de 15 pages depuis le modèle neutre, renseigne les données de la fiche et intègre directement la signature ainsi que le tampon d'Intégrale Academy dans le PDF. Yousign ne crée ensuite qu'un seul signataire : le formateur, avec authentification obligatoire par code SMS (`otp_sms`). Tant que la demande est en cours, le bouton **Renvoyer l'invitation de signature** déclenche une relance manuelle du même signataire sans créer de contrat en double. Si les données de la fiche ou la version du générateur changent, le PDF est régénéré avant le prochain envoi. Une demande active (`draft`, `approval`, `ongoing` ou `partially_signed`) n'est pas recréée afin d'éviter les doublons, sauf une ancienne demande à deux signataires qui peut être annulée et remplacée depuis la fiche. Les contrats formateurs APS peuvent aussi être envoyés depuis la page session.

L'application stocke l'identifiant du signataire formateur et de son champ, le mode d'authentification demandé, le statut, les dates d'envoi/synchronisation/webhook, le lien de signature éventuel et la dernière erreur. Les anciens champs du signataire centre sont conservés dans le schéma uniquement pour relire les demandes historiques déjà créées à deux signataires. Les métadonnées des versions précédemment envoyées sont archivées lorsqu'un nouveau contrat est généré.
