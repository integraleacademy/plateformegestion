...
const ready=data.templates.filter(t=>t.status==='ready'||((t.isNew3||t.isMetier||t.isGeneral||t.isBts||t.isSeasonal)&&t.status==='preview'));
assert.ok(ready.length >= 115);
const newTemplates=ready.filter(template=>template.isNew);
assert.equal(newTemplates.length,55);
assert.equal(new Set(newTemplates.map(template=>template.id)).size,55);
assert.equal(new Set(newTemplates.map(template=>template.previewStyle)).size,55);
...
for(const t of ready){const slide={templateId:t.id,content,options:{showSafeMargins:false}};const node=renderSlide(project,slide,'export',{templates:ready,templateRegistry:registry,themes:{A3P:{}}});assert.equal(node.dataset.renderedTemplateId,t.id);assert.equal(node.dataset.templateFamily,t.family);assert.equal(node.dataset.studioFormat,'1080x1080');assert.equal(node.dataset.canvasWidth,'1080');assert.equal(node.dataset.canvasHeight,'1080');assert.equal(node.style['--studio-logo-size'],'150px');assert.ok(node.querySelector('img[alt="Intégrale Academy"]'), `${t.id} logo`);assert.ok(node.textContent.includes('Faites le premier pas vers votre futur métier'), `${t.id} slogan`);assert.ok(node.innerHTML.includes('data-region="brand"'),`${t.id} brand region`);assert.ok(node.innerHTML.includes('data-region="footer"'),`${t.id} footer region`);assert.ok(node.innerHTML.includes('data-fit='),`${t.id} fitted text`);if(t.isNew)assert.ok(node.innerHTML.includes(`data-layout-role="brand-${t.brandLayout}"`),`${t.id} brand placement role`);const fp=getTemplateStructureFingerprint(node);assert.ok(fp.length>15, `${t.id} fingerprint`);fingerprints.set(t.id,fp)}
assert.equal(new Set(newTemplates.map(template=>fingerprints.get(template.id))).size,55);
const overrideProbe={dataset:{},querySelectorAll(){return []}};
const overrideSlide={templateId:'new_manifesto_highlight',elementOverrides:{'new-arrow-story--role-title':{x:40}}};
decorateEditableElements(overrideProbe,overrideSlide,{renderMode:'export'});
assert.equal(overrideProbe.dataset.studioManualLayout,'false');
overrideSlide.elementOverrides['new-manifesto-highlight--role-title']={x:20};
decorateEditableElements(overrideProbe,overrideSlide,{renderMode:'export'});
assert.equal(overrideProbe.dataset.studioManualLayout,'true');
