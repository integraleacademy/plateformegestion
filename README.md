...
- `YOUSIGN_CENTER_SIGNER_EMAIL` (optionnel) : adresse du signataire Intégrale Academy pour les contrats cadres formateurs, `ecole@integraleacademy.com` par défaut.
- `YOUSIGN_CENTER_SIGNER_FIRST_NAME` et `YOUSIGN_CENTER_SIGNER_LAST_NAME` (optionnels) : identité du signataire Intégrale Academy, `Clément VAILLANT` par défaut.

Workflow fiche formateur : compléter au minimum le nom, le prénom, l'adresse postale et le SIRET, puis utiliser **Générer le contrat** ou directement **Générer et envoyer pour signature**. L'application produit le contrat cadre de sous-traitance de 15 pages depuis le modèle neutre, renseigne les données de la fiche, conserve la version générée et l'envoie à Yousign. Le formateur signe avec un code SMS et le signataire Intégrale Academy reçoit également sa zone de signature. Si les données de la fiche changent, le PDF est régénéré avant le prochain envoi. Une demande active (`draft`, `approval`, `ongoing` ou `partially_signed`) n'est pas recréée afin d'éviter les doublons. Les contrats formateurs APS peuvent aussi être envoyés depuis la page session.

L'application stocke les identifiants Yousign, les deux identifiants de signataires et de champs, le statut, les dates d'envoi/synchronisation/webhook, le lien de signature éventuel et la dernière erreur. Les métadonnées des versions précédemment envoyées sont archivées lorsqu'un nouveau contrat est généré.
